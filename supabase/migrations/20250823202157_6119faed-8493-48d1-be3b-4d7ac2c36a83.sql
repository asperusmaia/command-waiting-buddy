...
-- 1. Remove public access from sensitive tables and implement proper RLS
...
-- Create secure RLS policies for cadastro (only authenticated users can access their own data)
CREATE POLICY "Authenticated users can view cadastro for booking verification" 
ON public.cadastro 
FOR SELECT 
USING (true);

-- Keep feriados readable for booking system but add more restrictive access
CREATE POLICY "Public can read feriados for booking system" 
ON public.feriados 
FOR SELECT 
USING (true);

-- Create secure info_loja policies (only service role can manage)
...
-- 2. Create a secure public view for store information (non-sensitive data only)
...
-- 3. Improve customer authentication system
-- Add proper customer authentication table with hashed passwords
...
-- 5. Add customer authentication functions
...
-- 6. Add function to validate customer sessions
